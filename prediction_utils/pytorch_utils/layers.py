...
class EmbeddingBagLinear(nn.Module):
    ...
    def compute_offsets(self, batch):
        return (
            torch.LongTensor(batch.indices).to(self.embed.weight.device),
            torch.LongTensor(batch.indptr[:-1]).to(self.embed.weight.device)
            if batch.shape[0] > 1
            else torch.LongTensor([0]).to(self.embed.weight.device),
            torch.FloatTensor(batch.data).to(self.embed.weight.device),
        )
